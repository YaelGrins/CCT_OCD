...
def run_one_iter_of_tinynerf(
    ...
    for batch in batches:
        pred,lat = model(batch)
        latout = lat
        predictions.append(pred)
        lat_out.append(latout)
    radiance_field_flattened = torch.cat(predictions, dim=0)
    latent = torch.cat(lat_out, dim=0)
    # "Unflatten" to obtain the radiance field.
    unflattened_shape = list(query_points.shape[:-1]) + [4]
    radiance_field = torch.reshape(radiance_field_flattened, unflattened_shape)
    latent_unflat = torch.reshape(latent, list(query_points.shape[:-1])+[128])
    # Perform differentiable volume rendering to re-synthesize the RGB image.
    rgb_predicted, _, _, weights = render_volume_density(
        radiance_field, ray_origins, depth_values
    )
    latent = (weights[...,None]*latent_unflat).view(-1,128).sum(0)/weights.view(-1).sum()
    return rgb_predicted,(latent.unsqueeze(0))
...
def main():
    ...
    for i in trange(num_iters):
        ...
        if i % display_every == 0 or i == num_iters - 1:
            ...
            # plt.title("PSNR")
            # plt.show()
    torch.save(model.state_dict(),'checkpoint_tinynerf.pt')
    print("Done!")
