...
parser.add_argument(
    '-pb', '--backbone_path', type=str, default = './base_models/checkpoint_cct7.pth',
    help='checkpoint path for backbone, default </base_models/checkpoint_cct7.pth>')
parser.add_argument(
    '-pc', '--config_path', type=str, default = './configs/train_cct7.json',
    help='config path, default </configs/train_cct7.json>')
parser.add_argument(
...
elif config.training.loss == 'lsce':
    opt_error_loss = losses.LabelSmoothingCrossEntropy()
...
print("mat weight shape", mat_shape)
...
for idx, batch in enumerate(test_loader):
    ...
    print(f"\rBaseline loss {lbaseline/(idx+1)}, Overfitted loss {lopt/(idx+1)}, Diffusion loss {ldiff/(idx+1)}, accuracy: {correct_items/total_items}" ,end='')
print(f"Total Acc:{correct_items/total_items}")